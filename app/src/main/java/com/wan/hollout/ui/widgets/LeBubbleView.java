...
public class LeBubbleView extends RelativeLayout implements Runnable {
    ...
    private void initialize(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.LeBubbleView, defStyleAttr, R.style.LeBubbleTextView_Dark);
        float radius = a.getDimension(R.styleable.LeBubbleView_bubbleCornerRadius, 0);
        backgroundColor = a.getColor(R.styleable.LeBubbleView_bubbleBackgroundColor, 0);

        pressBackgroundColor = a.getColor(R.styleable.LeBubbleView_bubbleBackgroundPressColor, 0);
        int textColor = a.getColor(R.styleable.LeBubbleView_bubbleTextColor, 0);
        float textSize = a.getDimension(R.styleable.LeBubbleView_bubbleTextSize, 0);
        String contentText = a.getString(R.styleable.LeBubbleView_bubbleText);

        int intDirection = a.getInt(R.styleable.LeBubbleView_bubbleArrowDirection, 3);
        setCurDirection(intDirection);

        float relativePosition = a.getFraction(R.styleable.LeBubbleView_relativePosition, 1, 1, 0.3f);
    ...
    private void initContent(float radius, final int backgroundColor, int textColor, float textSize, String content) {
        ...
        switch (arrowDirection) {
            ...
            default: {
                r = 0;
                if (Build.VERSION.SDK_INT <= 16) {
                    arrowParams.addRule(RelativeLayout.RIGHT_OF, conRl.getId());
                } else {
        ...
        DrawableCompat.setTintList(arrowImage.getDrawable(), ColorStateList.valueOf(backgroundColor));
