...
public class RoundedDrawable extends Drawable {
    ...
    public static Bitmap drawableToBitmap(Drawable drawable) {
        try{
            ...
            try {
                ...
                e.printStackTrace();
                HolloutLogger.w(TAG, "Failed to create bitmap from drawable!");
                bitmap = null;
            }
    ...
    private void redrawBitmapForSquareCorners(Canvas canvas) {
        ...
        if (!mCornersRounded[HolloutRoundedImageCorner.TOP_LEFT]) {
            mSquareCornersRect.set(left, top, left + radius, top + radius);
            canvas.drawRect(mSquareCornersRect, mBitmapPaint);
        }

        if (!mCornersRounded[HolloutRoundedImageCorner.TOP_RIGHT]) {
            mSquareCornersRect.set(right - radius, top, right, radius);
            canvas.drawRect(mSquareCornersRect, mBitmapPaint);
        }

        if (!mCornersRounded[HolloutRoundedImageCorner.BOTTOM_RIGHT]) {
            mSquareCornersRect.set(right - radius, bottom - radius, right, bottom);
            canvas.drawRect(mSquareCornersRect, mBitmapPaint);
        }

        if (!mCornersRounded[HolloutRoundedImageCorner.BOTTOM_LEFT]) {
     ...
     * @return the {@link RoundedDrawable} for chaining.
     */
    public RoundedDrawable setCornerRadius(@HolloutRoundedImageCorner int corner, float radius) {
    ...
    public RoundedDrawable setCornerRadius(float topLeft, float topRight, float bottomRight,
        ...
        mCornersRounded[HolloutRoundedImageCorner.TOP_LEFT] = topLeft > 0;
        mCornersRounded[HolloutRoundedImageCorner.TOP_RIGHT] = topRight > 0;
        mCornersRounded[HolloutRoundedImageCorner.BOTTOM_RIGHT] = bottomRight > 0;
        mCornersRounded[HolloutRoundedImageCorner.BOTTOM_LEFT] = bottomLeft > 0;
        return this;
    }
